...
mod tests {
    ...
    fn create_test_manager(event_bus: Arc<EventBus>) -> ScreenManagerImpl<TestBackend> {
        let challenge_store =
            Arc::new(ChallengeStore::default()) as Arc<dyn ChallengeStoreInterface>;
        let repository_store =
            Arc::new(RepositoryStore::default()) as Arc<dyn RepositoryStoreInterface>;
    ...
    #[test]
    fn setup_event_subscriptions_stores_navigate_transition() {
        let event_bus = Arc::new(EventBus::new());
        let manager = Arc::new(Mutex::new(create_test_manager(event_bus.clone())));

        ScreenManagerImpl::setup_event_subscriptions(&manager);
        event_bus.publish(NavigateTo::Replace(ScreenType::Help));

        let manager = manager.lock().unwrap();
        let pending = manager.pending_transition.lock().unwrap().clone();
