...
impl ChallengeConverter {
    ...
    pub fn convert_whole_files_to_challenges(&self, file_paths: Vec<std::path::PathBuf>) -> Vec<Challenge> {
    ...
    fn split_chunk_by_difficulty(&self, chunk: &CodeChunk, difficulty: &super::super::game::stage_builder::DifficultyLevel) -> Vec<Challenge> {
        ...
        if break_point > 0 && break_point < lines.len() {
            ...
            let selected_lines: String = lines.iter()
                .take(break_point)
                .map(|l| format!("{}\n", l))
                .collect();
            ...
            if !selected_lines.trim().is_empty() {
                ...
                let truncated_code_chars = self.count_code_characters(truncated_content, &adjusted_comment_ranges);
                ...
                if truncated_code_chars >= min_chars {
                    ...
                    let challenge = Challenge::new(id, truncated_content.to_string())
                        .with_source_info(file_path, chunk.start_line, chunk.start_line + break_point - 1)
                        .with_language(language)
    ...
    fn count_code_characters(&self, content: &str, comment_ranges: &[(usize, usize)]) -> usize {
        ...
        for (i, ch) in chars.iter().enumerate() {
            ...
            let in_comment = comment_ranges.iter().any(|&(start, end)| i >= start && i < end);
    ...
    fn find_optimal_break_point(&self, content: &str, comment_ranges: &[(usize, usize)], target_chars: usize) -> usize {
        let lines: Vec<&str> = content.lines().collect();
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            for (char_idx, ch) in line.chars().enumerate() {
                ...
                let in_comment = comment_ranges.iter().any(|&(start, end)| char_pos >= start && char_pos < end);
    ...
    fn adjust_comment_ranges_for_truncation(&self, original_ranges: &[(usize, usize)], new_length: usize) -> Vec<(usize, usize)> {
        original_ranges.iter()
    ...
    pub fn convert_whole_files_to_zen_challenges_only(&self, file_paths: Vec<std::path::PathBuf>) -> Vec<Challenge> {
