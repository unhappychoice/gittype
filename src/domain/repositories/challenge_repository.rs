...
mod tests {
    ...
    impl ProgressReporter for CountingProgressReporter {
        ...
        fn set_file_counts(
            ...
            assert!(current_file
                .as_deref()
                .is_some_and(|file| file.starts_with("Reconstructing challenge")));
    ...
    fn create_test_repo(root_path: PathBuf) -> GitRepository {
        GitRepository {
            user_name: "test".to_string(),
            repository_name: "repo".to_string(),
            remote_url: "https://github.com/test/repo".to_string(),
            branch: Some("main".to_string()),
            commit_hash: Some("reconstruct-source-file".to_string()),
            is_dirty: false,
    ...
    #[test]
    fn save_then_load_reconstructs_challenge_from_source_file() {
        ...
        file_storage.set_file_content(source_path.canonicalize().unwrap(), source_content.into());
        ...
        let git_repo = create_test_repo(repo_root);
        let challenge =
            Challenge::new("from-source".to_string(), "stale cache content".to_string())
                .with_language("rust".to_string())
                .with_difficulty_level(DifficultyLevel::Normal)
                .with_source_info("src/lib.rs".to_string(), 2, 4)
                .with_comment_ranges(vec![(3, 8)]);
        let reporter = CountingProgressReporter::new();

        repository.save_challenges(&git_repo, &[challenge]).unwrap();

        let loaded = repository
            .load_challenges_with_progress(&git_repo, Some(&reporter))
            .unwrap();

        assert_eq!(loaded.len(), 1);
        assert_eq!(loaded[0].id, "from-source");
        assert_eq!(
            loaded[0].code_content,
            "fn target() {\n    println!(\"cached\");\n}"
        );
        assert_eq!(loaded[0].source_file_path.as_deref(), Some("src/lib.rs"));
        assert_eq!(loaded[0].start_line, Some(2));
        assert_eq!(loaded[0].end_line, Some(4));
        assert_eq!(loaded[0].comment_ranges, vec![(3, 8)]);
        assert_eq!(loaded[0].difficulty_level, Some(DifficultyLevel::Normal));
        assert_eq!(reporter.calls.load(Ordering::SeqCst), 1);
        assert_eq!(reporter.processed.load(Ordering::SeqCst), 1);
        assert_eq!(reporter.total.load(Ordering::SeqCst), 1);
    }
}
