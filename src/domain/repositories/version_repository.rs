...
mod tests {
    ...
    fn create_repository() -> VersionRepository {
        VersionRepository {
            github_client_factory: Arc::new(GitHubApiClientFactoryImpl::default()),
            file_storage: Arc::new(FileStorage::new()),
        }
    ...
    fn create_cache_entry(last_checked: chrono::DateTime<Utc>) -> VersionCacheEntry {
        VersionCacheEntry {
    ...
    #[test]
    fn is_cache_valid_accepts_recent_current_version_entry() {
        let repository = create_repository();
        let entry = create_cache_entry(Utc::now() - Duration::hours(1));

        assert!(repository.is_cache_valid(&entry, 24));
    }

    #[test]
    fn is_cache_valid_rejects_expired_entry() {
        let repository = create_repository();
        let entry = create_cache_entry(Utc::now() - Duration::hours(25));
