...
#[test]
fn ensure_challenge_returns_database_error_when_existing_rowid_has_wrong_type() {
    let db_impl = Database::new().unwrap();
    let db = Arc::new(db_impl) as Arc<dyn DatabaseInterface>;
    let dao = ChallengeDao::new(Arc::clone(&db));
    let conn = db.get_connection().unwrap();
    ...
    conn.execute("DROP TABLE challenges", []).unwrap();
    conn.execute(
        "CREATE VIEW challenges AS SELECT 'not-a-rowid' AS rowid, 'bad-rowid' AS id",
        [],
    )
    .unwrap();

    let tx = conn.unchecked_transaction().unwrap();
    let challenge = Challenge::new("bad-rowid".to_string(), "fn test() {}".to_string());
    let error = dao
        .ensure_challenge_in_transaction(&tx, &challenge)
        .unwrap_err();
