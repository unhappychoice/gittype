mod version_service_tests {
    ...
    async fn test_version_update_available() {
        ...
        let (has_update, current_version, latest_version) = service.check_with_version("0.8.0").await
        ...
        assert!(has_update, "Should have update available when current=0.8.0 and latest=1.0.0");
    }
    ...
    async fn test_version_up_to_date() {
        ...
        let (has_update, current_version, latest_version) = service.check_with_version("1.0.0").await
            .expect("Version check should succeed");

        assert_eq!(current_version, "1.0.0");
        assert_eq!(latest_version, "1.0.0");
        assert!(!has_update, "Should not have update when versions are equal");
    }
    ...
    async fn test_version_newer_than_latest() {
        ...
        let (has_update, current_version, latest_version) = service.check_with_version("2.0.0").await
            .expect("Version check should succeed");

        assert_eq!(current_version, "2.0.0");
        assert_eq!(latest_version, "1.0.0");
        assert!(!has_update, "Should not have update when current is newer than latest");
    }
    ...
    async fn test_patch_version_update() {
        ...
        let (has_update, current_version, latest_version) = service.check_with_version("0.9.9").await
            .expect("Version check should succeed");

        assert_eq!(current_version, "0.9.9");
        assert_eq!(latest_version, "1.0.0");
        assert!(has_update, "Should have update available when 0.9.9 < 1.0.0");
    }

    #[tokio::test]
    async fn test_service_creation() {
        let service = VersionService::new();
        assert!(service.is_ok(), "Service creation should succeed with mocks enabled");
    }
    ...
    async fn test_multiple_calls_consistency() {
        ...
        let result1 = service.check_with_version("0.8.0").await.expect("First check should succeed");
        let result2 = service.check_with_version("0.8.0").await.expect("Second check should succeed");
        ...
        assert_eq!(result1.0, result2.0, "Update availability should be consistent");
    }
}
