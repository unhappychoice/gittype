use gittype::scoring::RankingTitle;
...
fn test_identify_missing_ascii_art() {
    ...
    println!("  Have ASCII art: {} ({:.1}%)", has_art.len(), 
        (has_art.len() as f32 / titles.len() as f32) * 100.0);
    println!("  Missing ASCII art: {} ({:.1}%)", missing_art.len(),
    ...
    if missing_art.len() > 40 {
        panic!("Too many titles missing ASCII art: {} out of {}", missing_art.len(), titles.len());
    }
}